...
BOT_DESCRIPTION = (
    ...
    "• قیمت‌گذاری شفاف و رقابتی {p('money')}\n"
    "• مدیریت مصرف و سرعت فوق‌العاده {p('bolt')}\n"
    "• ارتباط پایدار و بدون قطعی {p('globe')}\n\n"
    "برای شروع کافیست /start را ارسال کنید."
)
...
CHANNEL_GATE_TEXT = (
    f"کاربر عزیز {p('rose')}\n\n"
    f"برای استفاده از ربات لطفاً با دکمه‌های زیر در کانال‌های ما عضو شوید {p('fire')}\n\n"
    f"بعد از عضو شدن روی دکمه «{p('confirm')}عضو شدم» کلیک کنید."
)
CHANNEL_GATE_NOT_JOINED = (
    f"{p('error')}هنوز در همه کانال‌ها عضو نشده‌اید.\n\n"
...
ERRORS = {
    ...
    "discount_used": f"{p('error')}شما قبلاً از این کد تخفیف استفاده کرده‌اید.",
    "banned": (
