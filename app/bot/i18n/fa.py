"""
All user-facing Persian strings in one place.
No string may appear in handlers directly — always import from here.
"""

# ─── Common ──────────────────────────────────────────────────────────────────

BACK = "◀️ بازگشت"
BACK_DOUBLE = "◀️◀️ بازگشت"
BACK_TO_MENU = "◀️ بازگشت به منوی اصلی"
HOME = "🏠 منوی اصلی"
CANCEL = "❌ لغو"
CONFIRM = "✅ تایید"
REJECT = "❌ رد کردن"
CLOSE = "✖️ بستن"
REFRESH = "🔄 بروزرسانی"
COMING_SOON = "🔜 این بخش به‌زودی فعال می‌شود."

# ─── Welcome & Main Menu ─────────────────────────────────────────────────────

WELCOME = (
    "✨ به فروشگاه <b>Nexoranode</b> خوش آمدید!\n\n"
    "🛡 ارائه انواع سرویس‌های VPN با کیفیت عالی\n"
    "🔒 تضمین امنیت ارتباطات شما\n"
    "🎧 پشتیبانی حرفه‌ای ۲۴ ساعته\n\n"
    "از منوی زیر بخش مورد نظر خود را انتخاب کنید."
)

MAIN_BTN_BUY = "🚀 خرید سرویس"
MAIN_BTN_CONFIGS = "📊 مدیریت کانفیگ‌ها"
MAIN_BTN_BALANCE = "💰 افزایش موجودی"
MAIN_BTN_ACCOUNT = "👤 حساب کاربری"
MAIN_BTN_FREE = "⭐ کانفیگ رایگان"
MAIN_BTN_SUPPORT = "🎧 ارتباط با پشتیبانی"
MAIN_BTN_APPS = "📥 دریافت اپلیکیشن‌ها"

# ─── Errors ──────────────────────────────────────────────────────────────────

ERRORS = {
    "general": "❌ خطایی رخ داد. لطفاً مجدداً تلاش کنید.",
    "api_error": "⚠️ خطا در ارتباط با سرور. لطفاً چند لحظه دیگر تلاش کنید.",
    "config_create_failed": "❌ خطا در ایجاد سرویس. تیم پشتیبانی در حال بررسی است.",
    "insufficient_balance": (
        "💰 موجودی کافی نیست!\n\n"
        "موجودی فعلی: <b>{balance}</b> تومان\n"
        "مبلغ مورد نیاز: <b>{required}</b> تومان\n"
        "کمبود: <b>{shortage}</b> تومان\n\n"
        "برای افزایش موجودی از منوی اصلی اقدام کنید."
    ),
    "service_name_taken": (
        "❌ این نام قبلاً استفاده شده است.\n"
        "نام دیگری انتخاب کنید."
    ),
    "service_name_invalid": (
        "❌ نام نامعتبر است!\n"
        "فقط حروف کوچک انگلیسی (a-z) و اعداد (0-9) مجاز است.\n"
        "طول نام باید بین ۳ تا ۳۰ کاراکتر باشد.\n\n"
        "دوباره تلاش کنید:"
    ),
    "invalid_discount": "❌ کد تخفیف نامعتبر یا منقضی شده است.",
    "discount_used": "❌ شما قبلاً از این کد تخفیف استفاده کرده‌اید.",
    "banned": "🚫 دسترسی شما محدود شده است.\nبرای پیگیری با پشتیبانی تماس بگیرید.",
    "admin_only": "🚫 این بخش فقط برای مدیران قابل دسترسی است.",
    "not_found": "❌ مورد درخواستی یافت نشد.",
    "config_not_found": "❌ سرویس مورد نظر یافت نشد.",
    "quantity_invalid": "❌ تعداد نامعتبر است. عددی بین {min} تا {max} وارد کنید.",
    "amount_invalid": "❌ مبلغ نامعتبر است. عدد صحیح وارد کنید.",
    "amount_min": "❌ حداقل مبلغ {min} تومان است.",
}

# ─── Buy: type ───────────────────────────────────────────────────────────────

BUY_TYPE_HEADER = "🛒 برای خرید سرویس لطفاً یکی از دسته‌بندی‌های زیر را انتخاب کنید."

BUY_VIP_BTN = "⭐ سرویس اختصاصی VIP"

# ─── Buy: plans table ────────────────────────────────────────────────────────

VIP_PLANS_TABLE_HEADER = "⭐ <b>سرویس اختصاصی VIP</b>\n"
VIP_PLANS_TABLE_ROW = "│ {gb:<5} │ {days:<5} │ {price:<11} │"
VIP_PLANS_TABLE_HEAD = "│ 🔋 حجم │ 🕐 زمان │ 💲 قیمت     │"
VIP_PLANS_TABLE_SEP = "├" + "─" * 9 + "┼" + "─" * 9 + "┼" + "─" * 13 + "┤"
VIP_PLANS_TABLE_TOP = "┌" + "─" * 9 + "┬" + "─" * 9 + "┬" + "─" * 13 + "┐"
VIP_PLANS_TABLE_BOTTOM = "└" + "─" * 9 + "┴" + "─" * 9 + "┴" + "─" * 13 + "┘"

VIP_PLAN_BTN = "{gb} گیگ — {price} تومان"

# ─── Buy: quantity ───────────────────────────────────────────────────────────

QUANTITY_PROMPT = (
    "🛒 <b>خرید تعدادی</b>\n\n"
    "🔄 محصول: تحویل آنی | {gb} گیگ | {days} روزه | {price} ت\n"
    "💲 قیمت هر عدد: {price} تومان\n\n"
    "──────────────────\n"
    "❓ چه تعداد می‌خواهید؟\n\n"
    "🔢 حداقل: ۱   |   حداکثر: {max}\n\n"
    "✏️ عدد موردنظر را تایپ کنید (مثلاً ۱، ۲، ۵)"
)

# ─── Buy: service name ───────────────────────────────────────────────────────

SERVICE_NAME_PROMPT = (
    "🏷 <b>نام سرویس</b>\n\n"
    "لطفاً نام دلخواه سرویس خود را وارد کنید.\n"
    "نام باید فقط شامل حروف کوچک انگلیسی و عدد باشد.\n\n"
    "✏️ مثال: <code>ali</code> یا <code>user1</code>\n\n"
    "یا دکمه «نام رندوم» را بزنید تا نام خودکار ساخته شود."
)
SERVICE_NAME_RANDOM_BTN = "🎲 نام رندوم"

SERVICE_NAME_MULTI_PROMPT = (
    "🏷 <b>نام سرویس‌ها</b>\n\n"
    "شما {n} سرویس انتخاب کرده‌اید.\n"
    "یک نام پایه وارد کنید — به انتهای آن شماره اضافه می‌شود.\n\n"
    "✏️ مثال: <code>ali</code> → ali-1، ali-2 …\n\n"
    "یا دکمه «نام رندوم» را بزنید."
)

# ─── Buy: discount ───────────────────────────────────────────────────────────

DISCOUNT_PROMPT = (
    "🎟️ <b>کد تخفیف ویژه</b>\n\n"
    "💰 مبلغ قابل پرداخت: <b>{amount}</b> تومان\n\n"
    "💎 پیش از پرداخت، اگر کد تخفیف اختصاصی دارید کد تخفیف را وارد کرده 🔑\n"
    "و از مزایای ویژه آن بهره‌مند شوید 🎁\n\n"
    "📋 لطفاً کد تخفیف خود را تایپ کرده و ارسال کنید:"
)
DISCOUNT_SKIP_BTN = "✖️ خیر، ادامه"

DISCOUNT_APPLIED = (
    "✅ کد تخفیف اعمال شد!\n"
    "💰 تخفیف: <b>{discount}</b> تومان\n"
    "💲 مبلغ جدید: <b>{new_amount}</b> تومان"
)

# ─── Buy: payment method ─────────────────────────────────────────────────────

PAYMENT_METHOD_HEADER = (
    "🔄 تحویل آنی | {gb} گیگ | {days} روزه | {price} ت\n"
    "💲 مبلغ قابل پرداخت: <b>{amount}</b> تومان\n\n"
    "روش پرداخت را انتخاب کنید:"
)
PAY_WALLET_BTN = "💵 پرداخت از موجودی"
PAY_CARD_BTN = "💳 کارت‌به‌کارت"

# ─── Buy: card payment ───────────────────────────────────────────────────────

CARD_PAYMENT = (
    "💳 <b>کارت به کارت</b>\n\n"
    "🏦 {bank}\n"
    "👤 {owner}\n"
    "💳 <code>{card}</code>\n\n"
    "──────────────────\n\n"
    "💲 مبلغ قابل پرداخت\n"
    "<b>{amount}</b> تومان\n\n"
    "──────────────────\n\n"
    "⚠️ حتماً مبلغ را به همین مقدار واریز نمایید.\n"
    "در صورت واریز مبلغ غیر دقیق، مسئولیت تایید نشدن\n"
    "رسید بر عهده خود شما خواهد بود.\n\n"
    "📸 پس از واریز، تصویر رسید را ارسال کنید."
)
AMOUNT_TOMAN_BTN = "💱 مبلغ تومان"
AMOUNT_RIAL_BTN = "🔢 مبلغ ریال"
CARD_NUMBER_BTN = "📋 شماره کارت"

AMOUNT_TOMAN_ALERT = "💱 مبلغ تومان: {amount}"
AMOUNT_RIAL_ALERT = "🔢 مبلغ ریال: {amount}"
CARD_NUMBER_ALERT = "💳 شماره کارت:\n{card}"

RECEIPT_RECEIVED = (
    "✅ رسید پرداخت دریافت شد!\n\n"
    "⏳ در حال بررسی توسط تیم پشتیبانی…\n"
    "معمولاً در کمتر از ۳۰ دقیقه تایید می‌شود.\n\n"
    "🔔 پس از تایید، سرویس شما فعال و اطلاع‌رسانی می‌شود."
)
RECEIPT_PROMPT = "📸 لطفاً تصویر رسید پرداخت را ارسال کنید."

# ─── Buy: success ────────────────────────────────────────────────────────────

PURCHASE_SUCCESS_ONE = (
    "🎉 <b>سرویس شما فعال شد!</b>\n\n"
    "🏷 نام سرویس: <b>{name}</b>\n"
    "📦 پلن: {plan_name} — {gb} گیگ | {days} روز\n"
    "📅 وضعیت: {expiry}\n\n"
    "🔗 <b>لینک اشتراک:</b>\n"
    "<code>{sub_url}</code>"
)
PURCHASE_SUCCESS_BULK = (
    "🎉 <b>{n} سرویس با موفقیت ایجاد شد!</b>\n\n"
    "{lines}\n\n"
    "از منوی اصلی → «مدیریت کانفیگ‌ها» جزئیات هر سرویس را ببینید."
)
PURCHASE_LINE = "🏷 <b>{name}</b> — {sub_url}"
PURCHASE_REJECTED = (
    "❌ <b>پرداخت شما رد شد.</b>\n\n"
    "{reason}\n\n"
    "در صورت نیاز با پشتیبانی تماس بگیرید."
)

# ─── Admin payment forward ───────────────────────────────────────────────────

ADMIN_PAYMENT_FWD = (
    "💳 <b>درخواست پرداخت جدید</b>  #TXN-{tx_id}\n\n"
    "👤 کاربر: {name} (@{username})\n"
    "🆔 آیدی: <code>{tg_id}</code>\n"
    "📦 پلن: {plan_name}\n"
    "🔢 تعداد: {quantity}\n"
    "🏷 نام سرویس: <b>{service_name}</b>\n"
    "💰 مبلغ: <b>{amount}</b> تومان\n"
    "🎟 تخفیف: {discount}\n"
    "🕐 زمان: {datetime}"
)
ADMIN_WALLET_FWD = (
    "💰 <b>درخواست شارژ کیف پول</b>  #TXN-{tx_id}\n\n"
    "👤 کاربر: {name} (@{username})\n"
    "🆔 آیدی: <code>{tg_id}</code>\n"
    "💵 مبلغ: <b>{amount}</b> تومان\n"
    "🕐 زمان: {datetime}"
)
ADMIN_APPROVE_BTN = "✅ تایید و ایجاد سرویس"
ADMIN_APPROVE_WALLET_BTN = "✅ تایید شارژ"
ADMIN_REJECT_BTN = "❌ رد کردن"

# ─── Manage Configs ──────────────────────────────────────────────────────────

CONFIGS_LIST_HEADER = (
    "📊 <b>مدیریت کانفیگ‌ها</b>\n\n"
    "تعداد: {count}"
)
CONFIGS_LIST_EMPTY = (
    "📊 <b>مدیریت کانفیگ‌ها</b>\n\n"
    "❌ شما هیچ سرویس فعالی ندارید.\n\n"
    "برای خرید سرویس از منوی اصلی اقدام کنید."
)
CONFIG_LIST_ROW = "… {name}"
CONFIG_LIST_ROW_EXPIRED = "💤 {name} (منقضی)"

CONFIG_DETAIL = (
    "🏷 نام سرویس: <b>{name}</b>\n"
    "⭐ نوع سرویس: {plan_name}\n"
    "🔋 حجم: <b>{total_gb}</b> گیگ\n"
    "🕐 مدت زمان: {duration}\n"
    "👥 تعداد کاربر: نامحدود\n\n"
    "🔗 <b>کانفیگ اتصال:</b>\n"
    "<code>{vless}</code>\n\n"
    "🔑 <b>لینک اشتراک:</b>\n"
    "<code>{sub_url}</code>"
)

CONFIG_BTN_USAGE = "📊 وضعیت و راهنمای سرویس"
CONFIG_BTN_GET_CONFIGS = "📋 دریافت کانفیگ‌ها"
CONFIG_BTN_GET_SUB = "📱 دریافت اشتراک"
CONFIG_BTN_DISABLE = "🚫 غیرفعال‌سازی موقت"
CONFIG_BTN_ENABLE = "▶️ فعال‌سازی"
CONFIG_BTN_DELETE = "🗑 حذف کانفیگ"
CONFIG_BTN_RESET_SUB = "🔄 تغییر لینک ساب"
CONFIG_BTN_QR = "📱 QR ساب"

CONFIG_INFO_TRAFFIC = "🔋 {used} از {total} گیگ"
CONFIG_INFO_UNLIMITED = "♾️ نامحدود"

CONFIG_STATUS_TEXT = (
    "📊 <b>وضعیت سرویس: {name}</b>\n\n"
    "📈 مصرف حجم:\n"
    "<code>[{bar}] {used_gb} از {total_gb} گیگ ({pct}٪)</code>\n\n"
    "📅 وضعیت انقضا: <b>{expiry}</b>\n"
    "⏳ روزهای باقیمانده: <b>{days}</b>\n\n"
    "🔼 آپلود: {up}\n"
    "🔽 دانلود: {down}"
)

CONFIG_GET_CONFIGS_TEXT = (
    "📋 <b>کانفیگ‌های سرویس: {name}</b>\n\n"
    "🌐 <b>کانفیگ WS:</b>\n"
    "<code>{ws}</code>\n\n"
    "🔐 <b>کانفیگ Reality:</b>\n"
    "<code>{reality}</code>"
)
CONFIG_GET_SUB_TEXT = (
    "🔑 <b>لینک اشتراک سرویس: {name}</b>\n\n"
    "<code>{url}</code>\n\n"
    "این لینک را در اپ خود وارد کنید.\n"
    "⚠️ لینک اشتراک خصوصی است — با دیگران به اشتراک نگذارید."
)
DELAYED_START_FMT = "⏳ هنوز شروع نشده — شروع {n} روز پس از اولین اتصال"
CONFIG_NOT_STARTED = "⏳ هنوز شروع نشده"

CONFIG_DISABLED = "⏸ سرویس موقتاً غیرفعال شد."
CONFIG_ENABLED = "▶️ سرویس مجدداً فعال شد."
CONFIG_RESET_SUB_DONE = (
    "🔄 لینک اشتراک تغییر کرد:\n"
    "<code>{url}</code>"
)
CONFIG_QR_CAPTION = "📱 QR کد سرویس: <b>{name}</b>"

CONFIG_DELETE_CONFIRM = (
    "⚠️ <b>آیا مطمئن هستید؟</b>\n\n"
    "سرویس «<b>{name}</b>» برای همیشه حذف خواهد شد.\n"
    "این عمل قابل بازگشت نیست."
)
CONFIG_DELETE_YES = "✅ بله، حذف شود"
CONFIG_DELETE_NO = "❌ خیر، بازگشت"
CONFIG_DELETED = "🗑 سرویس «{name}» حذف شد."

# ─── Account / Wallet ────────────────────────────────────────────────────────

PROFILE_TEXT = (
    "👤 <b>پروفایل کاربری</b>\n\n"
    "👤 نام: <b>{name}</b>\n"
    "🔑 نام کاربری: {username}\n"
    "🆔 آیدی: <code>{tg_id}</code>\n"
    "💲 موجودی: <b>{balance}</b> تومان"
)
WALLET_TOPUP_BTN = "💰 افزایش موجودی"
WALLET_TX_BTN = "📋 تراکنش‌های من"

TOPUP_AMOUNTS_HEADER = (
    "💰 <b>افزایش موجودی کیف پول</b>\n\n"
    "مبلغ موردنظر را انتخاب یا وارد کنید:"
)
TOPUP_CUSTOM_BTN = "✏️ مبلغ دلخواه"
TOPUP_CUSTOM_PROMPT = (
    "✏️ مبلغ مورد نظر را به تومان وارد کنید:\n"
    "(مثال: 100000)"
)

TX_LIST_HEADER = "📋 <b>تراکنش‌های من</b>\n"
TX_LIST_EMPTY = "📋 <b>تراکنش‌های من</b>\n\nهنوز تراکنشی ثبت نشده است."
TX_LIST_ROW = (
    "──────────────────\n"
    "{icon} {desc}\n"
    "💰 {sign}{amount} تومان\n"
    "📅 {date}"
)
TX_ICON_CREDIT = "✅"
TX_ICON_DEBIT = "❌"
TX_ICON_PENDING = "⏳"
TX_ICON_REFERRAL = "🎁"

WALLET_CHARGED = (
    "✅ <b>موجودی شما با موفقیت شارژ شد!</b>\n\n"
    "موجودی جدید: <b>{balance}</b> تومان"
)

# ─── Referral / Free Config ──────────────────────────────────────────────────

REFERRAL_WITH_STATS = (
    "👥 <b>زیرمجموعه‌گیری ♥ و دعوت دوستان</b>\n\n"
    "با دعوت دوستان از طریق لینک اختصاصی، بدون پرداختی تا ۱\n"
    "ریال کیف پول شارژ می‌شه و از خدمات ربات استفاده می‌کنی! 🔥\n\n"
    "🎁 هدیه خرید: به ازای هر ۱ خرید زیرمجموعه، <b>{ref_bonus}</b> تومان شارژ\n"
    "کیف پول\n"
    "🎀 هدیه دعوت‌شونده: دوستان دعوت‌شده <b>{friend_bonus}</b> تومان\n"
    "موجودی کیف پول می‌گیرند\n\n"
    "📊 آمار شما:\n"
    "👥 زیرمجموعه‌ها: <b>{count}</b> نفر\n"
    "🛍 خریدهای زیرمجموعه: <b>{purchases}</b> عدد\n"
    "💰 مجموع خرید زیرمجموعه: <b>{total_revenue}</b> تومان\n\n"
    "🔗 <b>لینک دعوت شما:</b>\n"
    "<code>{ref_link}</code>"
)
REFERRAL_NO_STATS = (
    "🎁 <b>فرصت ویژه برای دریافت کانفیگ رایگان!</b>\n\n"
    "🤩 این ربات به‌صورت رایگان به شما کانفیگ میده\n"
    "کاملاً تست‌شده و قابل استفاده 👇\n\n"
    "💡 برای دریافتش کافیه از لینک زیر وارد بشی و فقط استارت کنی،\n"
    "هیچ هزینه‌ای نداره و خیلی راحت می‌تونی استفاده کنی 🙏\n\n"
    "🚀 فرصت رو از دست نده!\n\n"
    "🔗 <b>لینک ربات:</b>\n"
    "<code>{ref_link}</code>"
)
REFERRAL_SHARE_BTN = "➕ اشتراک‌گذاری لینک"
REFERRAL_POST_BTN = "📋 دریافت پست آماده"
REFERRAL_SHARE_DIALOG_TEXT = "🔥 VPN رایگان از نکسورانود"

REFERRAL_READY_POST = (
    "🔥 <b>VPN پرسرعت با قیمت مناسب!</b>\n\n"
    "✅ سرعت بالا\n"
    "✅ پایداری ۱۰۰٪\n"
    "✅ پشتیبانی ۲۴ ساعته\n"
    "✅ سازگار با تمام دستگاه‌ها\n\n"
    "🎁 با لینک من، سرویس رایگان دریافت کن:\n"
    "{ref_link}"
)
REFERRAL_READY_POST_HINT = "👆 پست آماده شد! می‌توانید آن را فوروارد کنید."

# ─── Apps ────────────────────────────────────────────────────────────────────

APPS_HEADER = (
    "📥 <b>دریافت اپلیکیشن‌ها</b>\n\n"
    "سیستم‌عامل خود را انتخاب کنید:"
)
APPS_OS_BTN = {
    "android": "🤖 Android",
    "ios": "🍎 iOS",
    "windows": "🪟 Windows",
    "mac": "🍏 Mac",
    "linux": "🐧 Linux",
}
APPS_OS_HEADER = "📱 <b>اپلیکیشن‌های {os}</b>\n\nبرنامه مورد نظر را انتخاب کنید:"

# ─── Support ─────────────────────────────────────────────────────────────────

SUPPORT_HEADER = (
    "🎧 <b>پشتیبانی</b>\n\n"
    "از گزینه‌های زیر انتخاب کنید:"
)
SUPPORT_FAQ_BTN = "❓ سوالات متداول"
SUPPORT_ONLINE_BTN = "⭐ پشتیبانی آنلاین ↗️"

FAQ_TEXT = (
    "📋 <b>سوالات متداول | Nexoranode VPN</b>\n\n"
    "💡 پاسخ کامل به سوالات پرتکرار شما عزیزان:\n\n"
    "──────────────────\n"
    "1️⃣ <b>سرویس تست دارید؟</b>\n"
    "❌ خیر، به‌دلیل پروتکل‌های امنیتی سرویس تست ارائه نمیشه.\n\n"
    "──────────────────\n"
    "2️⃣ <b>سرویس‌ها محدودیت زمانی یا کاربر دارن؟</b>\n"
    "😊 خیر، هیچ‌گونه محدودیت کاربر و زمانی برای سرویس‌ها تعیین\n"
    "نشده و همه سرویس‌ها بدون محدودیت هستن.\n\n"
    "──────────────────\n"
    "3️⃣ <b>سرعت و پایداری چطوره؟ قطعی دارید؟</b>\n"
    "✅ هیچ سرویسی ۱۰۰٪ بدون اختلال نیست\n"
    "✅ ما شفاف میگیم: اختلالات احتمالی برای آپدیت سرورها\n"
    "آی‌پی کوتاه‌مدته هست. (حدود ۱۰ تا ۱۵ دقیقه)\n"
    "🚀 استفاده از سرورهای قدرتمند با پهنای باند بالا\n"
    "⚡ نتیجه: سرعت بالا و پایداری مناسب\n\n"
    "──────────────────\n"
    "4️⃣ <b>با چه برنامه‌هایی سازگاره؟</b>\n"
    "📱 V2rayNG | NPV Tunnel | V2Box | Hiddify | …\n\n"
    "──────────────────\n"
    "5️⃣ <b>سرویس شما روی کدوم اپراتورها وصل می‌شه؟</b>\n"
    "✅ هیچ تفاوتی وجود نداره و سرویس‌ها با همه اپراتورها\n"
    "در همه مناطق کشور سازگار هستن.\n\n"
    "──────────────────\n"
    "6️⃣ <b>سرویس‌های شما ضریب داره؟</b>\n"
    "✅ خیر، هیچ ضریبی رو سرویس‌ها تعیین نشده و کانفیگ شما\n"
    "همیشه با لینک ساب کنترل کامل بر حجم مصرفی خودتون\n"
    "داشته باشید 🙏\n\n"
    "──────────────────\n"
    "💌 هر سوالی داشتی، پشتیبانی در خدمتته ❤️\n"
    "توجه داشته باشد که فروش از طریق ربات @{bot_username} انجام\n"
    "میشه، برای خرید به اکانت پشتیبانی پیام ندید.\n\n"
    "@{support_username}"
)

# ─── Admin dashboard ─────────────────────────────────────────────────────────

ADMIN_DASHBOARD = (
    "🔐 <b>پنل مدیریت نکسورانود</b>\n\n"
    "📊 <b>آمار امروز:</b>\n"
    "• کاربران جدید: {today_users}\n"
    "• درآمد امروز: {today_revenue} تومان\n\n"
    "📈 <b>آمار کلی:</b>\n"
    "• کل کاربران: {total_users}\n"
    "• سرویس‌های فعال: {active_configs}\n"
    "• کل درآمد: {total_revenue} تومان\n\n"
    "🖥 <b>وضعیت سرور:</b>\n"
    "• CPU: {cpu}٪\n"
    "• RAM: {ram}٪\n"
    "• Xray: {xray_state}"
)

ADMIN_DISCOUNT_CREATED = (
    "✅ <b>کد تخفیف ایجاد شد!</b>\n\n"
    "🎟 کد: <code>{code}</code>\n"
    "💰 تخفیف: {value}\n"
    "🔢 حداکثر استفاده: {max_uses}\n"
    "📅 انقضا: {expires}"
)
ADMIN_DISCOUNT_LIST_HEADER = "🎟 <b>کدهای تخفیف فعال</b>\n"
ADMIN_DISCOUNT_ROW = (
    "──────────────────\n"
    "🎟 <code>{code}</code>\n"
    "💰 {value}\n"
    "🔢 {used}/{max_uses}\n"
    "📅 انقضا: {expires}"
)
ADMIN_DISCOUNT_NOT_FOUND = "❌ کد تخفیف پیدا نشد."
ADMIN_DISCOUNT_DEACTIVATED = "🗑 کد <code>{code}</code> غیرفعال شد."
ADMIN_DISCOUNT_STATS = (
    "📊 <b>آمار کد {code}</b>\n\n"
    "🔢 استفاده شده: {used}/{max_uses}\n"
    "📅 ساخته شده: {created}\n"
    "📅 انقضا: {expires}\n"
    "وضعیت: {state}"
)
ADMIN_DISCOUNT_USAGE_HELP = (
    "استفاده:\n"
    "<code>/addcode CODE PERCENT|AMOUNT USES EXPIRE_DAYS</code>\n\n"
    "مثال:\n"
    "<code>/addcode SUMMER10 10% 100 7</code>\n"
    "<code>/addcode FLAT5000 5000t 50 2</code>"
)

# ─── Notifications ───────────────────────────────────────────────────────────

NOTIF_EXPIRY_WARNING = (
    "⚠️ <b>سرویس شما رو به اتمام است!</b>\n\n"
    "🏷 نام: {name}\n"
    "📅 انقضا: {expiry} ({days} روز دیگر)\n"
    "📊 حجم باقیمانده: {remaining_gb} گیگ\n\n"
    "برای ادامه استفاده، یک سرویس جدید تهیه کنید."
)
NOTIF_TRAFFIC_WARNING = (
    "⚠️ <b>حجم سرویس شما رو به اتمام است!</b>\n\n"
    "🏷 نام: {name}\n"
    "📊 مصرف: {used_gb} از {total_gb} گیگ ({pct}٪)\n\n"
    "برای ادامه استفاده، سرویس جدید تهیه کنید."
)
NOTIF_NEW_CONFIG_BTN = "🚀 خرید سرویس جدید"

# ─── Misc UI strings ─────────────────────────────────────────────────────────

WAIT_CREATING = "⏳ در حال ایجاد سرویس…"
WAIT_PROCESSING = "⏳ در حال پردازش…"
TX_DESC_PURCHASE = "خرید {plan_name} × {qty} ({name})"
TX_DESC_WALLET = "شارژ کیف پول"
TX_DESC_REFERRAL_RECEIVED = "هدیه معرفی دوست"
TX_DESC_REFERRAL_FRIEND = "هدیه ورود با لینک معرف"
